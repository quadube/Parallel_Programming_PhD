...
#define N 32 // 2 of them are halos, does not work with odd size
#define TSTEPS 10000
...
void runJacobi1DCUDA(int tsteps, int n, double *A, double *B, double *A_outputFromGpu, double *B_outputFromGpu)
{
    ...
    cudaSetDevice(1);
    cudaMalloc(&d_A1, (n/2+1) * sizeof(double));                                   // FIXME
    cudaMalloc(&d_B1, (n/2+1) * sizeof(double));                                   // FIXME
    cudaMemcpy(d_A1, &A[n/2-1], (n/2+1) * sizeof(double), cudaMemcpyHostToDevice); // FIXME
    cudaMemcpy(d_B1, &B[n/2-1], (n/2+1) * sizeof(double), cudaMemcpyHostToDevice); // FIXME
    ...
    for (int t = 0; t < tsteps; ++t)
    {
        ...
        double *tmp1 = d_A1; // FIXME
        d_A1 = d_B1;         // FIXME
        d_B1 = tmp1;         // FIXME

        // ---> update halos directly using cudaMemcpyPeer peer to peer transfers version 1 <--- //
        cudaMemcpyPeer(&d_A1[0], 1, &d_A0[n/2-1], 0, sizeof(double));
        cudaMemcpyPeer(&d_A0[n/2], 0, &d_A1[1], 1, sizeof(double));

        // ---> update halos directly using cudaMemcpy transfers version 2 <--- //
        // cudaMemcpy(&d_A1[0], &d_A0[n/2-1], sizeof(double), cudaMemcpyDeviceToDevice);
        // cudaMemcpy(&d_A0[n/2], &d_A1[1], sizeof(double), cudaMemcpyDeviceToDevice);

        // ---> update through host versSion 3<--- //
        // double h_l, h_r;
        // cudaMemcpy(&h_r, &d_A0[n/2-1], sizeof(double), cudaMemcpyDeviceToHost);
        // cudaMemcpy(&h_l, &d_A1[1], sizeof(double), cudaMemcpyDeviceToHost);
        // cudaMemcpy(&d_A1[0], &h_r, sizeof(double), cudaMemcpyHostToDevice);
        // cudaMemcpy(&d_A0[n/2], &h_l, sizeof(double), cudaMemcpyHostToDevice);
    }
...
FIXME
136    cudaSetDevice(1);
137    cudaMalloc(&d_A1, (n/2+1) * sizeof(double));                                       
138    cudaMalloc(&d_B1, (n/2+1) * sizeof(double));                                       
139    cudaMemcpy(d_A1, &A[n/2-1], (n/2+1) * sizeof(double), cudaMemcpyHostToDevice); 
140    cudaMemcpy(d_B1, &B[n/2-1], (n/2+1) * sizeof(double), cudaMemcpyHostToDevice);

151    runJacobiCUDA_kernel<<<grid, block>>>(n/2+1, d_A1, d_B1);

156    cudaDeviceSynchronize();

162    double *tmp1 = d_A1;
163    d_A1 = d_B1;         
164    d_B1 = tmp1;         

__________________________________________________________________________
Version 1
167   cudaMemcpyPeer(&d_A1[0], 1, &d_A0[n/2-1], 0, sizeof(double));
168   cudaMemcpyPeer(&d_A0[n/2], 0, &d_A1[1], 1, sizeof(double));

[rm.barbosa@deeplearning01 10_MultiGPUs_singleThread]$ ./jacobi
...
__________________________________________________________________________
Version 2
171   cudaMemcpy(&d_A1[0], &d_A0[n/2-1], sizeof(double), cudaMemcpyDeviceToDevice);
172   cudaMemcpy(&d_A0[n/2], &d_A1[1], sizeof(double), cudaMemcpyDeviceToDevice);
...
Version 3
175   double h_l, h_r;
176   cudaMemcpy(&d_A0[n/2], &h_l, sizeof(double), cudaMemcpyHostToDevice);
177   cudaMemcpy(&d_A1[0], &h_r, sizeof(double), cudaMemcpyHostToDevice);
178   cudaMemcpy(&h_l, &d_A1[1], sizeof(double), cudaMemcpyDeviceToHost);
179   cudaMemcpy(&h_r, &d_A0[n/2-1], sizeof(double), cudaMemcpyDeviceToHost);
