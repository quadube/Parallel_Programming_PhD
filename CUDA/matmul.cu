...
TODO 1 - NAIVE implementation
______________________________

22  double sum = 0.0;

25  for (int k = 0; k< N; k++)
26    sum += A[row * N + k] * B[k * N + col];

29  C[row * N + col] = sum;

[rm.barbosa@deeplearning01 7_MatMul]$ ./matmul 
Timing simple implementation... done.
Timing optimized implementation... done.
Result = PASS
Matrix size: 2048x2048
Tile size: 16x16
Throughput of simple kernel: 2784.99 GFLOPS
Throughput of optimized kernel: 998049 GFLOPS
Performance improvement: 358.367x

___________________________

TODO 2 - optimized version
___________________________

42 __shared__ float ATile[TILE_DIM][TILE_DIM];
43 __shared__ float BTile[TILE_DIM][TILE_DIM];

47   int col = bx*blockDim.x + tx;

55   ATile[ty][tx] = A[row*N + (p*TILE_DIM + tx)];
56   BTile[ty][tx] = B[(p*TILE_DIM + ty)*N + col];

60  __syncthreads();

65   result += ATile[ty][k] * BTile[k][tx];

70  __syncthreads();

74  C[row * N + col] = result;
...
Matrix size: 2048x2048
Tile size: 16x16
Throughput of simple kernel: 3021.98 GFLOPS
Throughput of optimized kernel: 4698.55 GFLOPS
Performance improvement: 1.55479x
